...
- **Caveat:** the Social Alerts observation in Section 2/4 is explicitly weaker evidence than a formal audit. It is a single-file read made for an unrelated reason (fixing its badge), not a targeted investigation with a regression test proving the claim. Treat it as "probably fine, not yet verified to the same standard as the other 8 workspaces." **2026-08-06 closure: this caveat is stale.** Sprint 5 ran the formal audit this caveat calls for and closed it -- see `docs/SPRINT_5_CLOSEOUT_2026_07_22.md` (line 48: "Dedicated audit + regression test this sprint (`AlertsSection.test.tsx`) confirms it is a synchronous component with no fetch, cannot hang"). Independently re-confirmed live in this repository today: `src/features/alerts/AlertsSection.tsx` still has no top-level `useEffect`/`fetch` (only a user-triggered async handler for "Convert to task", not an initial-load fetch), matching the closed finding, and `src/features/alerts/AlertsSection.test.tsx` exists as the dedicated regression test.
