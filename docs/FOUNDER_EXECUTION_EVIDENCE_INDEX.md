...
- **`docs/readiness/ACTIONABLES_READINESS_MATRIX.md`** -- now 34 named actionables (A-01 through A-34; A-26 through A-34 added 2026-07-25 from a live HITL Settings/Knowledge-Hub walkthrough) across Single Tenancy, Multi-Tenancy, Enterprise Beta, Live Workflow, and Security/Compliance readiness states. Status vocabulary is `Yes` / `Blocked` / `No` / `Deferred`, each with a numeric confidence score and a "Last Updated" evidence citation. As of the last update (2026-07-25): **10 actionables `Yes`, 15 `Blocked`, 9 `No`** (of which 8 are confirmed defects found live, not merely untested) -- the walkthrough closed A-07 (profile editing, real), downgraded A-08 (invitation email delivery confirmed broken), and surfaced 9 new findings including a live-tenant demo-data leak on the Organization tab, a session-reuse security gap on "Welcome Aboard," and two minor UX-redundancy items. None of the new defect findings have been fixed yet -- founder instructed "log, do not act."
