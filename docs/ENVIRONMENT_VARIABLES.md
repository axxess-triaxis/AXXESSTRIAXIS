...
| `AXXESS_LITE_HOSTS` (optional) | Comma-separated host list overriding the hardcoded default (`lite.triaxisventures.com`, `triaxis-product-lite-web.vercel.app`) that `src/config/liteSurfaceHosts.ts` uses to recognize a Lite request. Not required for correct behavior today -- only set this if a Lite host is added/changed. |
| `AXXESS_SURFACE` (optional, XL-4, 2026-08-05) | One of `lite`, `x0`, `demo`. An explicit, additive surface declaration -- setting `lite` locks that entire Vercel project down to `/lite` routes and the Lite API allowlist, **regardless of which domain the request arrived on**, and forces demo mode off server-side. **Only ever set this to `lite` on `triaxis-product-lite-web`.** Setting it on any other project (`triaxis-www-frontend-import`, `triaxis-product-investor-demo`, `axxesstriaxis`) would restrict that entire deployment to `/lite` for every visitor -- do not copy this value between projects. Safe to leave unset everywhere; host-based detection (the default, zero-config behavior) already covers `triaxis-product-lite-web` correctly without it. |
...
| `AXXESS_PLATFORM_OPERATOR_ORGANIZATION_ID` (Sprint 1 pilot portfolio, 2026-08-15) | Triaxis Ventures' own real `organizations.id`. The sole gate for `GET /api/admin/pilot-portfolio`'s cross-tenant rollup (`canViewCrossTenantPilotPortfolio` in `src/security/rbac.ts`) -- unset means the route always returns 403, for every user, including a real Super Admin. Never set this to any other tenant's org id. |
| `SENTRY_AUTH_TOKEN`, `SENTRY_ORG_SLUG`, `SENTRY_PROJECT_SLUG` | Optional. Powers the real Sentry project-health tile on `/admin/product-analytics` (`src/services/integrations/sentryProjectHealth.ts`). Absent -> honest "Not connected," never fabricated. |
