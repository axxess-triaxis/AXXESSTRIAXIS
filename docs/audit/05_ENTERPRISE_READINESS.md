...
| **Audit logging -- AI-agent approval actions** | **GAP** | The MCP agent tool-call approval flow (`api/approvals/[id]/route.ts`) -- the mechanism that lets a human gate what an autonomous AI agent can do to tenant data -- writes zero entries to the `audit_logs` table used everywhere else as the system of record. No DB trigger covers it either. Who approved which agent action, for which connection, resulting in what standing grant, is reconstructable only from a different, non-audit-log table with no immutability protection (this phase, #5). |
...
| **Data retention & deletion** | **CRITICAL GAP** | Both the account-deletion and data-export endpoints are one-line stubs that record a canned "will be processed manually" message and do nothing else -- one doesn't even write an audit log entry for itself. A genuinely well-designed erasure-planning module (`src/privacy/privacyEngine.ts`) exists but has zero production callers (confirmed only a type-only import exists anywhere). No auto-purge/retention-period logic runs anywhere despite a `retention_days` column existing in schema. Real, working `ON DELETE CASCADE` behavior exists at the database level, but no application code path ever triggers the delete that would exercise it (this phase, #2). |
...
**Question:** The account-deletion request endpoint (`src/app/api/account/deletion-request/route.ts`) and the data-export request endpoint (`src/app/api/privacy/export-request/route.ts`) both perform zero actual data operations -- they record a canned message stating the request will be manually processed, and one of them doesn't even write an audit log entry for itself. A separately-built erasure-planning module (`src/privacy/privacyEngine.ts`) that would do the real work (tombstone DB rows, delete storage objects, delete vector chunks, purge caches) exists but is never called by any production code path -- confirmed by tracing every importer of that module. Is this a known, deliberate "beta-stage manual process" (i.e., deletion/export requests genuinely are handled by a human out-of-band today, just not through this code), or is this an unaddressed gap that hasn't been prioritized yet?
...
**Current evidence:** `src/app/api/account/deletion-request/route.ts` (26 lines, full file traced), `src/app/api/privacy/export-request/route.ts` (13 lines, full file traced), `src/privacy/privacyEngine.ts` (real planning logic, zero non-type-only production importers), `docs/PRIVACY_ENGINEERING.md`'s own "Operational Gaps" section (self-reports this as still required).
...
**Question:** The MCP agent tool-call approval flow (`src/app/api/approvals/[id]/route.ts`) -- where a human approves or rejects what an autonomous AI agent is allowed to do to tenant data, and optionally grants standing "Always Allow" permission -- writes zero entries to the `audit_logs` table that every other privileged action in the app uses as its system of record. No database trigger covers this table either. Is this intentional (e.g., `approval_requests`/`agent_action_grants` were considered sufficient records on their own), or should this flow write to `audit_logs` the same way role changes and invitations do?
...
**Current evidence:** `src/app/api/approvals/[id]/route.ts` (full file traced, zero `auditLogsRepository` calls), `src/repositories/workflowActionRepositories.ts:239-257` (`approvalRequestsRepository.decide()`, writes only to `approval_requests` via service-role client), `src/services/agents/agentGrantsRepository.ts:50-70` (`createGrant`, same pattern), migration search confirming no audit trigger exists on either table.

**Possible interpretations:**
A. Deliberate -- `approval_requests`/`agent_action_grants` were considered adequate as their own record, and adding a redundant `audit_logs` write wasn't prioritized.
...
**What evidence would resolve it:** Founder confirmation, and/or adding an `auditLogsRepository.record()` call to the approval-decide and grant-create code paths.
...
- Two CRITICAL GAP findings: the RAG service-role isolation bypass (Q-005, carried forward from Phase 3, now formally scored) and the non-functional data-erasure/export pipeline (newly found this phase, Q-006).
