...
### Post-Sprint 41: Live Tenant 0 Onboarding, Product Issue 1 (2026-07-23)

`docs/BETA_QA_5_SPRINT_REMEDIATION_CHECKLIST_2026_07_22.md`'s "authenticated live golden-path replay" follow-up above is now underway, performed by the user directly as Triaxis Ventures attempts to onboard as Pilot Tenant 0. The first finding -- no discoverable sign-up entry point on `/auth`, no OAuth (Google/Microsoft) options anywhere, and a completely missing OAuth callback session handler -- was diagnosed against the live site and the source code, fixed (`src/features/auth/OAuthProviderButtons.tsx`, `establishServerSessionFromOAuthTokens` in `src/auth/serverSession.ts`, new `POST /api/auth/oauth/callback` route, hash-token capture in `EnterpriseAuthFlowPage`), tested (12 new/extended tests, 116 files / 362 tests total), committed (`32350c9`), and deployed to production. The deploy itself took three attempts: the first failed with an 11-hour vitest worker timeout from an overnight environment suspension (never reached the actual deploy step); the second completed cleanly (`dpl_A6FnntX928iN57E1iNkKJzEUkL7o`, `READY`) but its own `vercel inspect` output didn't list `beta.triaxisventures.com` in the alias display; a direct `curl` and browser screenshot against the live URL confirmed the new UI was being served there regardless. **Confirmed live and verified visually** as of this entry. Full detail, including what still requires external, non-code setup (real Google/Microsoft OAuth app registration) and what remains open (a generic, unhelpful sign-in error message found in the same investigation): `docs/TENANT_0_ONBOARDING_FINDINGS_2026_07_22.md`.
