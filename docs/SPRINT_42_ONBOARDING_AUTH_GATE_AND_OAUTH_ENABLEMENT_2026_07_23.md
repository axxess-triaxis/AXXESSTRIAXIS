...
- **Still open, unrelated to this sprint:** the two-tenant isolation harness has never been executed against a live database; GitHub's 22 Dependabot vulnerabilities remain uninvestigated.
- **Addendum (2026-07-23, post-closeout):** the Supabase transactional confirmation email question above is now resolved. A live re-walkthrough (`docs/TENANT_0_ONBOARDING_FINDINGS_2026_07_22.md`, "Attempt 3 Log") confirmed the confirmation email is deliverable end to end, including a working confirmation link. That same walkthrough reproduced Product Issue 2's "Unauthorized" result again on production, as expected -- this sprint's fix was not yet deployed at the time of that test -- and surfaced one new, separate finding: the onboarding wizard's security-notices screen doesn't enforce that all 4 notices are accepted before continuing, and the final provisioning-blocked message doesn't say which requirement failed. Not fixed in this sprint; documented only, per the "Attempt 3 Log" entry.
