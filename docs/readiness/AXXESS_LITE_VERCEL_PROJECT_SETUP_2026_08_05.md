...
**2026-08-22 update: this non-negotiable is now explicitly reversed by founder decision.** The live Lite site had gone 16 days stale (last real commit `f29642f`, 2026-08-06) precisely because no automated path existed. Founder explicitly requested automated production deployment for `lite.triaxisventures.com` be added, matching the `landing`/`investor-demo` pattern -- a `deploy-lite` job now exists in `.github/workflows/deploy-production.yml`, gated on the same `push: main` trigger, sequential after `deploy-investor-demo` (deliberately not parallel, given this session's own fresh evidence that concurrent/rapid Vercel deploys can trip both build-concurrency and daily-deployment-count limits). Requires a new `VERCEL_PROJECT_ID_LITE` repo secret (founder-provided; the CLI/dashboard project ID for `triaxis-product-lite-web`) -- until that secret is added, the job will fail cleanly at its "Pull Vercel environment" step, which is the expected, self-explanatory signature, not a bug. This decision does not change Lite's own feature-completeness (Files/Projects/Programs/Approvals remain scaffold; Integrations/Analytics/Billing remain unbuilt) -- Lite is now continuously deployed like the other two public surfaces, but that is a CI/CD scope change, not a claim that the product surface itself is finished. Recorded per this repo's decision-ledger discipline (`CLAUDE.md`).
