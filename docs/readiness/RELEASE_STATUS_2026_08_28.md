...
**New this session, not yet in any prior doc:** Lite's shell (`LiteShell.tsx`) had **no sign-out control anywhere in the product** -- the header showed the display name as plain text with no click target, and none of the five new Settings pages had one either. Founder caught this live. Fixed same session: a "Sign out" icon button wired to `AuthProvider`'s existing `logout()` (the same call X0's `TopBar` makes), test coverage added (`LiteShell.test.tsx`), `typecheck`/`lint` clean, targeted test passing (3/3); full suite run was in progress at the time this document was written -- **not yet committed pending that suite's result.**

**Evidence attached:** PR [#340](https://github.com/axxess-triaxis/AXXESSTRIAXIS/pull/340), PR [#341](https://github.com/axxess-triaxis/AXXESSTRIAXIS/pull/341), PR [#342](https://github.com/axxess-triaxis/AXXESSTRIAXIS/pull/342) (merged, closes ledger #70), founder's live screenshots (this session, unrecorded as separate files).
...
**Owner/action:** commit and merge the LiteShell sign-out fix once the full suite confirms clean; consider whether Organization/Profile need a similar "did we miss an obvious control" pass given this was found live, not in review.
...
- **Web Lite:** land the sign-out fix; build a self-serve pricing page and dedicated acquisition channel distinct from the general waitlist.
