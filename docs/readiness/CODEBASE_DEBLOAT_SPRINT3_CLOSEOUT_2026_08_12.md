...
3. **No checksum computed for any evidence artifact.** Deferred to the actual move step, so a
...
1. **Founder decision, still the single remaining blocker**: name the external storage destination.
   ...
   and all 4 LOI/engagement-letter documents) can execute, and this index's `External location`
   fields get filled in.
2. If ever needed at higher confidence, inspect the remaining 18 product-survey response PDFs
...
letter documents missing from the repo) is now fully resolved: all 4 located and reviewed in full,
and correctly kept out of the public repo -- the 3 LOIs once found to be unmasked (contrary to the
founder's initial belief), and the Sakura letter by consistent policy even though its own risk
profile was lower. Zero bytes moved, zero destination chosen -- both correctly deferred to the one
