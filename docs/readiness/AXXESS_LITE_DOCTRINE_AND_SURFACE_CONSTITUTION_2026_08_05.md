...
| XLA-22 | Wire `lite:mobile:validate` (and the rest of `lite:ci`) into CI as a required PR gate | X Lite Web, X Lite Mobile (protection) | Claude Code | New `.github/workflows/mobile-lite-capacitor.yml`, scoped to Lite-relevant paths, parallel to (not replacing) `mobile-capacitor.yml` | Workflow exists and runs `lite:guard`/`lite:test`/`lite:mobile:validate` on relevant PRs | **Done, workflow added (2026-08-05)** -- `lite:guard` (26 files), `lite:test` (34 tests), `lite:mobile:validate` all verified passing individually via direct invocation. The composite `lite:ci` script itself could not be end-to-end verified in this local sandbox (bare `pnpm` isn't on `PATH` here, only `corepack pnpm` -- a local environment quirk, not expected to affect GitHub Actions, which installs pnpm natively via `pnpm/action-setup`). Not yet observed running in a real PR. |
| XLA-23 | Generate real native Android/iOS projects for `apps/mobile-lite-capacitor` | X Lite Mobile | Founder/engineer with a verified Android SDK/Xcode toolchain | `npx cap add android` / `npx cap add ios` run successfully, `android/`/`ios/` directories committed | **Blocked -- waiting on credentials/toolchain access**, per founder instruction (2026-08-05): hold until XLA-24/XLA-25 are unblocked, since generating native projects without a resolved app identity or signing path invites rework |
| XLA-24 | Set up separate Android keystore + iOS certificates/provisioning profile for AXXESS Lite Mobile, distinct from X0 Mobile's | X Lite Mobile | Founder | Keystore and certificates exist, distinct from X0's, `CAPACITOR_LITE_APP_ID` locked to a founder-confirmed final value | **Blocked -- waiting on credentials**, per founder instruction (2026-08-05). Also depends on founder confirming the final Lite bundle ID (currently `com.triaxisventures.axxesslite`, marked not-final in `apps/mobile-lite-capacitor/capacitor.config.ts`) |
| XLA-25 | Register a separate AXXESS Lite app listing in Google Play Console and Apple App Store Connect | X Lite Mobile | Founder | Two new, distinct app records exist, separate from X0 Mobile's listings | **Blocked -- waiting on credentials**, per founder instruction (2026-08-05). Also gated on the existing D-U-N-S Number clearance (applied 2026-07-13, ~30-day TAT, expected ~2026-08-12) already blocking X0 Mobile's own company-name store release -- this is the same blocker extended to a second app, not a new one |
| XLA-26 | Make `apps/lite-web` a genuine standalone build target: extract the concrete Supabase-backed repositories + `applicationServices` proxy into `packages/core` (the ~5,000-line closure reaching into the AI provider system, demo dataset, and X0's routing types -- Phase 1a already moved the safe, ~907-line type-only subset, see `docs/readiness/LITE_WEB_PACKAGES_CORE_EXTRACTION_PHASE1A_CLOSEOUT_2026_08_06.md`), then build `apps/lite-web` a real independent Next.js page tree importing from it; only after both are done and verified, reconfigure the `triaxis-product-lite-web` Vercel project's Root Directory from `.` to `apps/lite-web` | X Lite Web | Claude Code (extraction + build), Founder (Vercel Root Directory change sign-off) | Dedicated session with a working, completable test loop (this sandbox's `pnpm run test` does not currently run to completion -- a real regression net is a precondition, not optional, given the closure touches live X0's request path for 5 real tenants) | `apps/lite-web`'s own build script produces an independent artifact (no `pnpm --dir ../.. run build` delegation), the Vercel project deploys from `apps/lite-web` as Root Directory, and the deployed Lite site is confirmed live and functionally unchanged | **Deferred, explicit founder decision (2026-08-06)** -- founder reviewed the real dependency-closure size and chose to store this as a tracked actionable rather than attempt it inline in an already-long mixed-task session. Not blocked on anything external; blocked only on scheduling a dedicated pass |

Fifteen to twenty-five actionables were requested; twenty-six are now listed above (twenty from XL-0, one added during XL-1's own spike, four added following up on this session's mobile-boundary discussion, one added 2026-08-06 -- XLA-26, the standalone-build-target deferral), spanning XL-1 (route/nav/shell), XL-2 (daily-use loop wiring), and XL-3 (payments/mobile/deployment) scope, plus the two protective checklists (XLA-17/18) and the founder-facing decisions (XLA-01, XLA-12, XLA-13) that gate the others. As of XL-0 none were marked `Ready` or `Done`, since XL-0 was a doctrine-only sprint; XL-1's own status update is recorded in this document's Section 8 ADR addendum and in `docs/readiness/AXXESS_LITE_PRODUCT_SURFACE_ROADMAP_2026_08_05.md`'s Section 9 checklist, not by rewriting this table's `Status` column sprint-by-sprint (that would require reconciling two sources of status truth; the checklist in the roadmap document is the authoritative per-sprint status tracker going forward, this table remains the actionable definitions).
