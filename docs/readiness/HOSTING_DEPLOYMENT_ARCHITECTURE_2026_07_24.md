...
**Attempted 2026-07-25:** the CLI's `vercel project update` still does not expose a Production Branch flag (confirmed via `--help`), and the CLI's beta `vercel api` command failed to accept a literal endpoint path in this environment. A direct call to the Vercel REST API (`PATCH /v9/projects/{id}` with `{"link":{"productionBranch":"canonical/sprint-1-35-unified-gitlab"}}`, using the already-authenticated CLI's own stored token) was attempted for all three projects and is the correct, tested-as-readable mechanism -- the read-side (`GET`) of this same call succeeded and returned the current settings shown above. The write-side (`PATCH`) was blocked by this session's own auto-mode permission classifier before it could execute, the same restriction that has applied to `git push` throughout this session. **Not yet applied as of this writing** -- see the founder-facing summary for how to authorize it.
