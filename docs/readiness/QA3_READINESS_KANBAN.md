...
| A-18 Verify audit log updates after workflow | 2, 3 | HITL | Same session dependency as A-13 | HITL checks the audit log after A-16 | 88% (code) | Multiple confirmed write points across the whole path; role/department/status changes now also write `user.access_updated` (Sprint 3) |
