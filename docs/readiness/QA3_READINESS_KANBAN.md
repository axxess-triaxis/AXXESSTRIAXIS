...
| A-26 Verify sign-in OAuth (Google/Microsoft) on the live auth page | -- | HITL (external provider registration) | Distinct from A-21 -- this is the sign-in-with-Google/Microsoft path on `/auth`, gated behind external provider console registration plus `NEXT_PUBLIC_AUTH_GOOGLE_ENABLED`/`NEXT_PUBLIC_AUTH_MICROSOFT_ENABLED` | HITL completes external OAuth app registration; Claude Code sets the two env vars once confirmed | -- | HITL-reported 2026-07-25: still does not work on `landing.triaxisventures.com` |
...
| A-55 RAG answers return templated/dummy-pattern text, not a real synthesized answer | -- | Claude Code (needs investigation) | Query against a real uploaded document returned "Tenant 0 dummy data" + echoed query keywords | Code-level investigation to distinguish stub-response-generator vs. citing-a-stale-placeholder-source (see A-61/A-62) | -- | HITL-reported 2026-07-25 (AI Workspace walkthrough). Log only |
| A-56 RAG confidence score is an opaque "black box" | -- | Claude Code | No visible explanation of the confidence computation | Surface or document the scoring logic | -- | HITL-reported 2026-07-25. Founder: "that '72% confidence' logic should not be 'black box'." Log only |
| A-57 AI Review Inbox "Create Stakeholder Note" + "Escalate" did not produce a visible CRM record | -- | Claude Code | Confirmed real for "Create Task" path, not confirmed for stakeholder-note/escalate path | Investigate the escalate/stakeholder-note action handler | -- | HITL-reported 2026-07-25. Log only |
| A-58 Stakeholders & CRM "Create Contact" auto-populates fixed Influence (50)/Engagement (medium) | -- | Claude Code | Values never entered by the user | Leave unset/user-specified, or compute from real signal | -- | HITL-reported 2026-07-25. Log only |
| A-59 "Review Approval Queue" routes to Analytics & Dashboard instead of Pending Approvals | -- | Claude Code | Confirmed wrong destination | Fix the route | -- | HITL-reported 2026-07-25. Log only |
| A-60 "Export Report" (Approvals & Governance) is a clickable placeholder | -- | Claude Code | Distinct from the Analytics & Reports "Export Report," which the founder explicitly deferred | Wire to real export or remove | -- | HITL-reported 2026-07-25. Log only |
| A-61 Document indexing UX: Knowledge Hub uploads don't auto-appear as indexing candidates | -- | Claude Code (product decision + build) | Only indexing input is "paste governable text," impractical for large documents | Add a HITL-triggered "select from Knowledge Hub documents" option | -- | HITL-reported 2026-07-25. Founder: "Not every document on 'Knowledge Hub' should be auto-indexed (should be HITL)." Likely contributes to A-55. Log only |
| A-62 Stale "Pitch deck" placeholder document remains in the governed RAG index | -- | Claude Code | Only source both RAG queries cited; content literally says "Tenant 0 dummy data" | Remove stale entry; index real documents (A-61) | -- | HITL-reported 2026-07-25. Founder: "This governed RAG doc is redundant, supposed to go." Directly related to A-55. Log only |
| A-63 Unclear whether "Create Task/Approval from Answer" carries RAG answer content into the created record | -- | Claude Code (needs investigation) | Behavior not confirmed either way | Code-level read of both action handlers | -- | HITL-reported 2026-07-25. Log only |
| A-64 "Ask AI Workspace" routes to Tasks & Workflow instead of AI Workspace | -- | Claude Code | Confirmed wrong destination | Fix the route | -- | HITL-reported 2026-07-25. Founder: "Totally incorrect... incorrect workflow." Log only |
| A-65 "Send Feedback" should notify triaxisgrp@gmail.com | -- | Claude Code (needs verification) | Real `beta_feedback` table write confirmed (ED-1); email notification unconfirmed | Check current config against this requirement | -- | HITL-reported 2026-07-25. Log only |
