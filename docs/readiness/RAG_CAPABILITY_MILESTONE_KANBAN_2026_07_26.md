...
- [x] Knowledge Hub document selector real, not paste-text-only (A-61, code-shipped)
...
## Current Status: 6 of 10 exit criteria code-complete and tested; 3 require a live HITL retest; 1 is a known, documented, out-of-scope capability gap

## Board

### Code-Complete and Tested (6 of 10 -- verified via typecheck/lint/test/build each sprint)

| Item | Evidence | Source |
|---|---|---|
| A-61 -- Knowledge Hub document selector | `DocumentsSection.tsx` selector + `ingestTenantDocument(documentId)` reindex path; `DocumentsSection.test.tsx` | Sprint 1 closeout, commit `0ed228e` |
...
### Code-Shipped, Pending Live HITL Retest (3)

| Card | What's shipped | What's still open | Priority |
|---|---|---|---|
| A-55 | Generator proven grounded at the code level | Whether the SPECIFIC "Tenant 0 dummy data" symptom is resolved for a real query against a real document requires a live re-query this environment cannot perform | High |
| A-61/A-62 | Selector built; archive-exclusion fixed | HITL must actually archive the stale document in production and confirm the selector + exclusion work end to end | High |
| A-13 | Composite of the above -- citation/confidence/HITL-routing mechanics already confirmed real in the original 2026-07-25 walkthrough | Full re-score to `Yes` depends on the A-55/A-61/A-62 retest above | High |
...
1. **HITL performs the 8-step ordered validation script** in
   `RAG_REMEDIATION_FINAL_EVIDENCE_PACKAGE_2026_07_26.md` -- covers all three sprints' live-retest
   requirements in one pass rather than three separate sessions.
2. **Re-score A-55, A-61, A-62, A-13** in `ACTIONABLES_READINESS_MATRIX.md` based on the retest's
   actual result -- do not pre-commit to `Yes` before the retest happens.
