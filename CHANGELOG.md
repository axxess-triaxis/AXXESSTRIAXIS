...
- Sprint 42 (Onboarding Auth Gate, Sign-Up/Login Diagnosability, OAuth Enablement Runbook): fixed "Product Issue 2" (`docs/TENANT_0_ONBOARDING_FINDINGS_2026_07_22.md`) -- the `/onboarding` wizard let a completely unauthenticated visitor fill out all 5 data-entry screens before failing only at the final "Provision tenant" click with a bare "Unauthorized". Added `/onboarding` to `protectedRoutePrefixes` in `src/proxy.ts` (edge-level redirect to `/auth?next=/onboarding`), plus a client-side `useAuth()` guard in `EnterpriseOnboardingPage.tsx` (reusing `src/app/App.tsx`'s existing "Sign in required" pattern) as defense-in-depth against a session expiring mid-wizard. Also fixed the linked, still-open "Create account" gap's root diagnosability problem: `src/app/api/auth/sign-up/route.ts` and `src/app/api/auth/login/route.ts` previously swallowed every Supabase Auth error with a bare `catch {}` and no server-side logging; both now parse the real error via a new `src/auth/supabaseAuthError.ts` helper, log it, and return specific codes for `user_already_exists` (409, sign-in instead) and `email_not_confirmed` (401, with a new "Resend confirmation email" action wired to a new `POST /api/auth/resend-confirmation` route). Rewrote `docs/AUTH.md`'s OAuth section into a precise, step-numbered external-steps checklist for actually enabling Google/Microsoft sign-in (no new application code required there -- the UI and callback plumbing already worked; only Google Cloud Console/Azure Portal/Supabase dashboard registration, which only Triaxis Ventures can perform, remains). See `docs/SPRINT_42_ONBOARDING_AUTH_GATE_AND_OAUTH_ENABLEMENT_2026_07_23.md` for the full closeout.
- Added tests: `src/app/api/auth/sign-up/route.test.ts`, `src/app/api/auth/login/route.test.ts`, `src/app/api/auth/resend-confirmation/route.test.ts`, `src/features/onboarding/EnterpriseOnboardingPage.test.tsx` (all new), plus extended cases in `src/proxy.test.ts`, `src/app/auth/page.test.tsx`, and `src/features/auth/EnterpriseAuthFlowPage.test.tsx`. Full suite: 120 files / 383 tests passing (up from 119/377 immediately after the auth-error-handling change alone).
- Verification for Sprint 42: `pnpm run typecheck` clean, `pnpm --dir apps/mobile run typecheck` clean, `pnpm run lint` clean (zero warnings), `pnpm run test` passing, `pnpm run build` succeeded (116 static pages), `pnpm run supabase:verify` passed (unchanged: 27 migrations, 100 RLS-protected tables, no schema change).

- Added a "Provenance" section to `README.md` recording that the hands-on technical build of AXXESS TRIaxis was executed solely by Sudipta Koushik Sarmah, Founder & Managing Director of Triaxis Ventures Private Limited, using AI assistant tools/agents as the only technical assistance. Updated to also record Triaxis Ventures' co-founder, Ritashree Mahanta -- a seasoned healthcare and education professional (6+ years, 4 Indian states) who has contributed company/product strategy, healthcare/education-sector workflow feedback and brainstorming, beta feedback collection from the healthcare and education sectors, 15+ stakeholder consultations and 10+ client scoping calls, and the company's first cheque (approximately 70% of her income over the last 6-7 months toward Triaxis Ventures' and AXXESS TRIaxis's operating runway). The same attestation is recorded in `docs/SPRINT_41_QA2_MILESTONE_2026_07_22.md` and `docs/TENANT_0_ONBOARDING_FINDINGS_2026_07_22.md`.

- Fixed "Product Issue 1" (found during a live Tenant 0 onboarding attempt): the sign-in page had no visible Sign Up entry point at all, and no alternative to manual email/password anywhere in the UI, even though backend OAuth-start infrastructure already existed unused. Added a visible "Sign up" link on `/auth`, and "Continue with Google"/"Continue with Microsoft" buttons on both `/auth` and `/auth/sign-up` (`src/features/auth/OAuthProviderButtons.tsx`). Built the previously-missing OAuth callback session establishment end to end: `establishServerSessionFromOAuthTokens` (`src/auth/serverSession.ts`), a new `POST /api/auth/oauth/callback` route, and hash-fragment token capture in `EnterpriseAuthFlowPage`'s `login` kind -- without this, the OAuth buttons would have redirected to Google/Microsoft but never actually completed a session. Google/Microsoft sign-in will not functionally work until Triaxis Ventures registers real OAuth apps in Google Cloud Console and Azure Portal and configures the resulting credentials in Supabase's dashboard -- an external, non-code step documented in `docs/AUTH.md`, not something further code changes can resolve.
- Added tests: `src/features/auth/OAuthProviderButtons.test.tsx` (3), `src/features/auth/EnterpriseAuthFlowPage.test.tsx` (5, new file), `src/app/api/auth/oauth/callback/route.test.ts` (4, new file), plus 1 new case in `src/app/auth/page.test.tsx`.

- Sprint 5 (Live QA Replay, Tenant Isolation, Audit Evidence Expansion And Release Gate): a live, read-only browser replay against `beta.triaxisventures.com` confirmed the production deployment was still running pre-Sprint-1 code -- a fresh cold browser rendered a full mock-authenticated dashboard while every tenant-scoped API call returned `401` (F-001/F-003), and the network log showed the dashboard's data hooks firing the same ~16 requests 2-3x each (F-021, worse than previously assumed). Traced to two causes: the live deployment predated this session's Sprint 1 work, and `NEXT_PUBLIC_AXXESS_AUTH_SHELL`/`NEXT_PUBLIC_AXXESS_DEMO_MODE` were never set on the Vercel project at all. With explicit user approval, set both variables and deployed to production via the Vercel CLI. Fixed F-021 (Actionable 20, the only one of 20 untouched through Sprint 4) by adding `src/hooks/liveWorkspaceMetricsCache.ts`, a tenant-scoped short-TTL cache so `useLiveWorkspaceMetrics`'s three independent call sites within `DashboardSection` (direct, plus via `useEnterpriseGoldenPath` and `useLiveRagHealth`) share one request instead of firing three. Performed the formal Social Alerts audit Sprint 3 deferred: found `AlertsSection.tsx` rendered 4 demo alerts and a hardcoded "4 active" badge completely unconditionally with no Demo Mode gate at all -- a genuine, previously undiscovered demo-data leak -- and fixed it to match the established Analytics/Stakeholders pattern. Generalized Sprint 2's projects-only audit/timeline evidence writer (`recordResourceCreateEvidence` in `src/app/api/repositories/[resource]/route.ts`) to also cover tasks, documents, knowledge articles and meetings. Wrote `scripts/verify-two-tenant-isolation.mjs`, an executable harness proving tenant isolation via real Supabase Auth users and real RLS (not application code) -- written and unit-tested, but not executed against a live database this sprint. Renamed `src/middleware.ts` to `src/proxy.ts` per Next.js 16's official middleware-to-proxy migration (confirmed safe, behavior-identical rename; build's deprecation warning is now gone). Confirmed the one remaining permissive-RLS warning (`public.permissions`'s `using (true)` select policy) is genuinely safe -- the table has no tenant/user column at all -- and documented this explicitly instead of leaving it unexplained.
- Added tests: `src/hooks/liveWorkspaceMetricsCache.test.ts` (4), `src/features/alerts/AlertsSection.test.tsx` (3, new), `scripts/verify-two-tenant-isolation.test.mjs` (7, new), `src/proxy.test.ts` (replaces `src/middleware.test.ts`), plus new cases in `src/app/api/repositories/[resource]/route.test.ts` and `src/auth/AuthProvider.test.tsx`. Full suite: 113 files / 349 tests passing (up from 110/331).
- Verification for Sprint 5: `pnpm run typecheck` clean, `pnpm --dir apps/mobile run typecheck` clean, `pnpm run lint` clean (zero warnings), `pnpm run test` passing, `pnpm run build` succeeded (middleware deprecation warning confirmed gone), `pnpm run supabase:verify` passed (27 migrations, 100 RLS-protected tables, no schema change), `pnpm run mobile:store:release-gate` passed, `pnpm run mobile:capacitor:store:doctor` passed. Doc updates: `docs/SPRINT_LOG.md`, `docs/SUPABASE_CLI.md`, `docs/AUTH.md`, `docs/VERCEL_DEPLOYMENT.md`, `docs/BETA_QA_ACTIONABLES_2026_07_22.md`, `docs/BETA_QA_5_SPRINT_REMEDIATION_CHECKLIST_2026_07_22.md`, `docs/BETA_QA_ANALYSIS_AND_REMEDIATION_ROADMAP_2026_07_22.md`, `docs/Post-Claude Code exhaustive workflow audit production remediation package.md`, `docs/SPRINT_1_TO_4_GAP_ANALYSIS_2026_07_22.md`.
