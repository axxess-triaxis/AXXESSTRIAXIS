...
function gmailMaxBodyCharacters() {
  const configured = Number(process.env.GMAIL_IMPORT_MAX_BODY_CHARS ?? 30000);
...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => ({})) as GmailImportRequest;
  const messageId = body.messageId?.trim();
  if (!messageId) return NextResponse.json({ error: "Gmail message id is required." }, { status: 400 });

  if (!isSupabaseAdminConfigured() || !isTokenVaultConfigured()) {
    return NextResponse.json({
      status: "provider_gated",
      providerId: "gmail",
      message: "Supabase service role access and AXXESS_TOKEN_VAULT_KEY are required for live Gmail selected-message import.",
    }, { status: 503 });
  }

  const connection = await findGmailConnection(session.user.organizationId, body.connectionId);
  if (!connection?.token_reference) {
    return NextResponse.json({
      status: "provider_gated",
      providerId: "gmail",
      message: "A connected Gmail account with an encrypted token vault reference is required first.",
    }, { status: 409 });
  }
  ...
  if (!vaultRow) {
    return NextResponse.json({
      status: "provider_gated",
      providerId: "gmail",
      message: "The Gmail token vault record is unavailable or revoked. Reconnect Gmail and retry.",
    }, { status: 409 });
  }
  ...
  const selectedEmail = await fetchGmailSelectedMessage({
    ...
    maxBodyCharacters: gmailMaxBodyCharacters(),
  });
  ...
  const document = await ingestTenantDocument({
    ...
    title: `Gmail import - ${selectedEmail.subject}`,
    bodyText: selectedEmail.bodyText,
    fileName: `${selectedEmail.messageId ?? Date.now()}-gmail-message.txt`,
    mimeType: "message/rfc822",
    visibility: "organization",
    classification: "internal",
    tags: ["gmail-import", "selected-message", ...preview.tags.slice(0, 4)],
  });

  const tasks = await Promise.all(preview.tasks.slice(0, 3).map((taskTitle) => tasksRepository.create(scope, {
    organizationId: scope.organizationId,
    title: taskTitle,
    description: `Created from selected Gmail message "${selectedEmail.subject}". Source: ${selectedEmail.sourceLink ?? "selected mailbox item"}`,
