...
// Q-006: the export-request route previously performed zero data operations -- no privacy_requests
// row, no execution plan, no audit log. This proves the honest queued-state fix: a real row is
// inserted with a real, computed execution plan, and the request is audit-logged.
const state = {
...
describe("POST /api/privacy/export-request (Q-006 honest queued-state fix)", () => {
  afterEach(() => {
    state.session = null;
  ...
  it("inserts a real privacy_requests row with a real, computed execution plan and audit-logs the request", async () => {
    ...
    expect(inserted.body.request_type).toBe("access_export");
    ...
    expect(logged.action).toBe("privacy.export.requested");
    expect(logged.resourceId).toBe("privacy-request-1");
  });

  it("still audit-logs and returns 200 when Supabase admin is not configured, with no requestId", async () => {
    state.session = { user: user("org-1") };
  ...
  it("never claims 'queued' when the insert itself fails -- Supabase admin is configured but the write throws", async () => {
    ...
    expect(body.ok).toBe(true);
    expect(body.requestId).toBeUndefined();
    expect(body.message).not.toMatch(/^Export request queued/i);
